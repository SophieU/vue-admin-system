    ...
    <Card style="padding-bottom: 30px;">
      <div class="mb-15 clearfix">
        ...
        <div class="pull-right">
          <Button @click="downloadExcel">Excel导入模板下载</Button>
          ...
          >
            <Button icon="ios-cloud-upload-outline">Excel模板导入</Button>
          </Upload>
      ...
      <div class="table-wrapper">
          <table class="native-table">
            ...
            <tbody>
              <tr v-for="(material,index) in materialCheckedlists" :key="material.materialId">
...
<script>
  ...
  import util from '../../libs/util';
    export default {
      ...
      computed:{
        uploadUrl:function(){
          let url = baseUrl+'repair/material/input/order/import';
          return url;
        },
      },
      methods:{
          downloadExcel(){
            this.$http.get(`/repair/material/input/order/download/template`,{responseType:'blob'})
              .then(res=>{
        ...
        uploadSuccess(res){
          setTimeout(()=>{
            if(res.code===0){
              ...
              //上传成功后处理
              let data = res.data;
              this.materialCheckedlists=data;
            }else{
