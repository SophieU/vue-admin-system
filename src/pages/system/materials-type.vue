        ...
        <div>
          ...
          <div class="table-wrapper">
            ...
            <div class="pagination">
              <Page  :total="totalCount" :current="pageNo" :page-size="pageSize" show-elevator
      ...
      >
        <div  class="two_column_form">
          <i-form ref="typeForm" :rules="typeFormRule" :model="typeForm" label-position="top">
            <div class="flex_form">
              <div class="left">
                <form-item class="must"  label="服务分类" >
                  <Select v-model="typeForm.repairCategoryParentId" @on-change="(id)=>repairFirstChange(id,'modal')">
                    <Option v-for="item in repairFirst" :key="item.id" :value="item.id">{{item.name}}</Option>
                  </Select>
                </form-item>
                <form-item label="服务项目" prop="repairCategoryId">
      ...
      <Drawer title="报修筛选"  v-model="filter" width="425" @on-close="clearFilter">
        ...
        <i-form v-model="materialsFilter" >
          <form-item label="服务分类">
            <Select v-model="materialsFilter.repairCategoryParentId" @on-change="(id)=>repairFirstChange(id,'filter')">
              <Option v-for="item in repairFirst" :key="item.id" :value="item.id">{{item.name}}</Option>
            </Select>
          </form-item>
    ...
    export default {
      ...
      data(){
          return {
            ...
            typeFormRule:{
              repairCategoryId:[{required:true,message:'请选择服务项目',trigger:'change'}],
            ...
            latestPrice:'',
            repairFirst:[], //服务分类-一级
            repairSecond:[], //服务分类-二级
          }
      },
      methods:{

        repairFirstChange(id,fromType,isInitData){
            ...
            this.$http.get(`/repair/material/category/info?id=${id}`)
              .then(res=>{
                if(res.data.code===0){
                  ...
                  this.repairFirstChange(this.typeForm.repairCategoryParentId,'modal',true);
