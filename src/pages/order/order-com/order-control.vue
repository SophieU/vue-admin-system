          ...
          <div class="control-flex">
            ...
            <div class="select-wrap">
              <Select style="width: 200px;display: block;" class="mb-15" @on-change="demandChange">
                <Option v-for="option in selectLists" :key="option.value" :value="option.value">{{option.reasonName}}</Option>
              </Select>
              <Select v-model="cancelReason" style="width: 200px;" v-if="currentCon==='cancel'" placeholder="请选择取消原因">
                <Option v-for="reason of reasonLists" :value="reason.id" :key="reason.id">{{reason.reasonName}}</Option>
              </Select>
            </div>
            <div class="btn-wrap">
          ...
          this.$http.post(`/repair/order/after/sale/add?id=${this.id}`)
            .then(res=>{
              if(res.data.code===0){
                ...
                this.$Message.error('发起失败:'+res.data.msg);
              }
            })
        },
        getCancelReason(){
