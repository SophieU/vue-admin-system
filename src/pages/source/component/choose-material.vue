<template>
  <Modal fullscreen  :closable="false" :mask-closable="false" title="选择辅材" v-model="showModal" width="650" @on-visible-change="modalVisible">
    <div>
      <div class="mb-15">
      ...
      <div class="table-wrapper">
        <table class="native-table">
          ...
          <tbody>
          <tr v-for="(material,index) in materialLists" :key="material.materialId">
        ...
        <div class="pagination">
          <Page :current.sync="searchForm.pageNo" @on-change="materialPageChange" size="small" :total="materialTotal" :page-size="15"></Page>
        </div>
      </div>
    </div>
    <div slot="footer">
          ...
          if(this.checkAll){
            ...
            this.materialLists.map((item,index)=>{
              ...
              this.materialCheckTemp=this.materialCheckTemp.filter((temp,index)=>{
                ...
                if(checkTempStr.indexOf(temp.materialId)<=-1){
                  return true;
                }
              });
            })
        ...
        checkChange(val){
          ...
          if(checked==='1'){
            ...
            this.materialCheckTemp.map((item,ind)=>{
              if(item.materialId===materialLists[index].materialId){
        ...
        compareChoosed(hasCheck,lists){
          ...
          lists=lists.map(item=>{
            hasCheck.forEach(check=>{
              if(item.materialId===check.materialId){
                item.disabled=true;
              }
            });
            if(!item.disabled){
